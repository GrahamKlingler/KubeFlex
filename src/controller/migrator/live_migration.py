...
def criu_migrate_pod(source_pod: str, source_node: str, target_node: str, namespace: str,
                     target_region: Optional[str] = None, delete_original: bool = True,
...
class CriuMigrationTracker:
    ...
    def __init__(self, source_pod: str, source_node: str, target_node: str, namespace: str, 
                 target_region: Optional[str] = None, delete_original: bool = True,
        ...
        self.base_pod_name = self._extract_base_pod_name(source_pod)
        
        # Determine target pod name using incrementing counter
        self.target_pod_name = self._get_next_pod_name(self.base_pod_name, namespace)
        logger.info(f"[MIGRATION] Using counter-based naming: {self.target_pod_name} (base: {self.base_pod_name})")
    ...
    def _get_next_pod_name(self, base_name: str, namespace: str) -> str:
        """
        Get the next pod name by finding the highest existing counter and incrementing.
        Returns base_name-1 if no existing pods found.
        """
        try:
            ...
            next_pod_name = f"{base_name}-{next_counter}"
            logger.info(f"[MIGRATION] Found max counter: {max_counter}, next pod name: {next_pod_name}")
            return next_pod_name
            
        except Exception as e:
            logger.warning(f"[MIGRATION] Failed to determine next pod name, using counter 1: {e}")
    ...
    def get_container_info_via_k8s_api(self, node_name: str, pod_name: str) -> Dict:
        """Get container information using Kubernetes API and shell commands."""
        try:
            self._update_step(f"getting_container_info_via_k8s_api_{node_name}", "container_discovery")
            
            load_kubernetes_config()
            api = client.CoreV1Api()
            
            # Get pod information using Kubernetes API
            ...
            logger.info(f"[K8S_API] Found pod UID: {pod_uid}")
            
            # Extract container image from pod spec
            container_image = None
            if pod.spec.containers:
                container_image = pod.spec.containers[0].image
                logger.info(f"[K8S_API] Container image: {container_image}")
            ...
            if pod.status.container_statuses:
                for container_status in pod.status.container_statuses:
                    if container_status.container_id:
                        # Container ID format: containerd://<container_id>
                        container_id = container_status.container_id.replace('containerd://', '')
                        logger.info(f"[K8S_API] Found container ID: {container_id}")
                        break
            ...
            container_info = {
                ...
                'name': pod.spec.containers[0].name if pod.spec.containers else None,
            ...
            return {
                ...
                'pod_id': pod_uid,  # Use pod UID instead of containerd pod ID
                'task_id': None,  # Not needed when using K8s API
                'container_info': container_info,
                'node': node_name
            }
            
        except client.rest.ApiException as e:
            logger.error(f"Kubernetes API error getting container info: {e}")
            return {}
        except Exception as e:
            logger.error(f"Failed to get container info via K8s API: {e}")
            import traceback
    ...
    def create_target_pod_only(self, container_info: Dict) -> str:
        ...
        try:
            ...
            # Use the target pod name (counter-based naming)
            container_name = self.target_pod_name
            # Get image from nested container_info dict
            image = container_info.get('container_info', {}).get('image', 'salamander1223/testpod:latest')
            if not image:
                image = 'salamander1223/testpod:latest'  # Fallback
            logger.info(f"[TARGET_POD] Using container image: {image}")
            ...
            # Add REGION label if target_region is provided
            if self.target_region:
                pod_spec_lines.append(f"    REGION: {self.target_region}")
            ...
            # Get the container ID using Kubernetes API
            load_kubernetes_config()
            api = client.CoreV1Api()
            
            # Wait a bit for container status to be populated
            max_retries = 10
            retry_count = 0
            new_container_id = None
            
            while retry_count < max_retries and not new_container_id:
                try:
                    pod = api.read_namespaced_pod(name=container_name, namespace=self.namespace)
                    
                    if pod.status.container_statuses:
                        for container_status in pod.status.container_statuses:
                            if container_status.container_id:
                                # Container ID format: containerd://<container_id>
                                new_container_id = container_status.container_id.replace('containerd://', '')
                                logger.info(f"[TARGET_POD] Found container ID via K8s API: {new_container_id}")
                                break
                    ...
                    if e.status != 404:
                        logger.warning(f"[TARGET_POD] K8s API error (retry {retry_count + 1}/{max_retries}): {e}")
            ...
            if not new_container_id:
                logger.error(f"Failed to get container ID for pod {container_name} on target node via K8s API")
                return None
    ...
    def perform_migration(self) -> bool:
        ...
        try:
            ...
            source_container_info = self.get_container_info_via_k8s_api(self.source_node, self.source_pod)
            ...
            if not self.transfer_script_data_to_target(self.target_pod_name):
            ...
            target_container_info = self.get_container_info_via_k8s_api(self.target_node, self.target_pod_name)
            
            logger.info("=" * 80)
            logger.info("[MIGRATION] Migration Summary:")
            logger.info(f"  Source Pod: {self.source_pod}")
            logger.info(f"  Target Pod: {self.target_pod_name}")
            ...
            # Step 11: Delete original pod if requested
            if self.delete_original:
                self._update_step("deleting_original_pod", "cleanup")
                logger.info("[MIGRATION] Deleting original pod...")
                self._delete_original_pod()
    ...
    def _delete_original_pod(self):
        ...
        try:
            ...
            self.migration_state['original_pod_deleted'] = False
            self.migration_state['errors'].append(f"Exception during pod deletion: {e}")
    ...
    def _add_migration_annotations(self):
        ...
        try:
            ...
            # Use the target pod name directly
            try:
                migrated_pod = api.read_namespaced_pod(name=self.target_pod_name, namespace=self.namespace)
            except client.ApiException as e:
                if e.status == 404:
                    logger.warning(f"[ANNOTATIONS] Could not find migrated pod {self.target_pod_name} to annotate")
                    return
                else:
                    raise
            ...
            annotations = {
                ...
                "target-region": self.target_region or "unknown",
                "migration-counter": str(self.target_pod_name.split('-')[-1]) if '-' in self.target_pod_name else "unknown"
            }
            ...
            logger.info(f"[ANNOTATIONS] Successfully added migration annotations to pod {self.target_pod_name}")
